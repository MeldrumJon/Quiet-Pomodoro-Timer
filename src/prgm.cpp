...
// Circle runs from 0 to 400
void eraseSegment(uint16_t from, uint16_t to) {
	uint16_t start, end;
	start = from % 50;
	if (from < 50) { goto tltop; }
	else if (from < 100) { goto tlbottom; }
...
// 	mylcd.Write_Cmd_Data(0x10, 0x2F8E); // Lower LCD power
...
void loop() 
{
	...
	for (uint8_t i = 1; i <= 12; ++i) {
		uint16_t end = (i*400)/12;
		eraseSegment(lastSeg, end);
		lastSeg = end;
		_delay_ms(500);
	}
	...
	_delay_ms(2000);
 	...
 	mylcd.Write_Cmd_Data(0x10, 0x7FF1); // Sleep mode!
 	
 	_delay_ms(15000);
 	
 	mylcd.Write_Cmd_Data(0x10, 0x7FD0); // The LCD has woken
